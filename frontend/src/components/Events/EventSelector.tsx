      ...
      {true && (
        <div className="event-selector">
          <div className="current-event-container" onClick={handleOpenDropdown}>
            <p className={`current-event-text ${eventList?.length > 1 && 'enable-dropdown'}`}>
              {eventContext?.name || 'Select an Event'}
            </p>
          </div>
          {dropDownOpen && (
            <div className="event-dropdown-list">
              {eventList?.map((e: any) => {
                return (
                  <p className="event-item" key={e?.id} onClick={() => handleSelectEvent(e)}>
                    {e?.name}
                  </p>
                );
              })}
